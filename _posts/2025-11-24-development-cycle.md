...
each bot uses a separate database file (`gronka-test.db` and `gronka-prod.db`) and can have independent configuration using prefixed environment variables (e.g., `TEST_ADMIN_USER_IDS`, `PROD_CDN_BASE_URL`). see the [configuration documentation](../_docs/configuration.md) for full details on prefixed variables.
